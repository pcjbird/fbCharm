...
class ViewController: UIViewController {
    ...
    func updateText() {
        ...
        let bLight = fontweightSwitch.isOn
        ...
        if !bFallback {
            if !bParagragh {
                self.label.font = UIFont(families: [], size: fontSize, weight: bLight ? .light : .regular)
                self.label.text = text
            }
            else{
                self.label.attributedText = NSMutableAttributedString(text: text, families: [], size: fontSize, weight: bLight ? .light : .regular, kern: bKern ? 4.0 : 0.0, paragraphStyle:paragraphStyle)
            }
        }
        else {
            if !bParagragh {
                self.label.font = UIFont(families: ["Lucida Grande", "Baskerville", "Apple SD Gothic Neo"], size: fontSize, weight: bLight ? .light : .regular)
                self.label.text = text
            }
            else{
                self.label.attributedText = NSMutableAttributedString(text: text, families: ["Lucida Grande", "Baskerville", "Apple SD Gothic Neo"], size: fontSize, weight: bLight ? .light : .regular, kern: bKern ? 4.0 : 0.0, paragraphStyle:paragraphStyle, fallback: bJPFallback ? "jp" : nil)
